...
class IndicatorViewPagerState extends State<IndicatorViewPager> {
  ...
  void controlTimer(){
    if(isContinue){
      count++;
      _controller.jumpToPage(count % widget._pages.length);
    }
  }

  void resumeTimeJump(var v){
    isContinue=true;
  }

  void resumeTimeJump1(){
    isContinue=true;
  }



  void pauseTimeJump(var v){
    isContinue=false;
  }
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      body: new IconTheme(
        ...
        child: GestureDetector(child: new Stack(
          children: <Widget>[
        ...
        ),onTapDown: pauseTimeJump,onTapUp: resumeTimeJump/*,onTapCancel: resumeTimeJump1,*/),
      ),
    );
  }
}
